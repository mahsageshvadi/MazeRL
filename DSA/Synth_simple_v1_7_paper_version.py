...
class CurveEnv:
    ...
    def step(self, a_idx: int):
        ...
        # ============ EPISODE TERMINATION (Paper Section 4.1.1) ============
        # (1) Track length exceeds 1.5x reference length
        ...
        # Mean min-distance from path to GT (per step)
        L_t = curve_to_curve_distance(self.path_points, self.ep.gt_poly)
        mean_d = L_t / max(len(self.path_points), 1)

        # Normalize: 1 when perfectly on-curve, ~0 when mean distance ~ D0
        ccs = 1.0 - (mean_d / (self.D0 + 1e-6))
        ccs = float(np.clip(ccs, -1.0, 1.0))  # keep it bounded

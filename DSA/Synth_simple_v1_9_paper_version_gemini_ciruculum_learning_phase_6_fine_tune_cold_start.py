...
def crop32(img: np.ndarray, cy: int, cx: int, size=CROP):
    h, w = img.shape
    # Smart Padding: Check corner/background color
...
# ---------- PHASE 6 ENV: COLD START LOGIC ----------
class CurveEnvColdStart:
    def __init__(self, h=128, w=128, max_steps=200):
        self.h, self.w = h, w
        self.max_steps = max_steps
        self.cm = CurveMakerFlexible(h=h, w=w, seed=None)
        # Default Config (Moderate/Hard)
        self.config = {'width': (2, 8), 'noise': 0.8, 'invert': 0.5, 'intensity': 0.2}
        self.reset()
    ...
    def reset(self):
        # 1. Generate Curve
        width = self.config.get('width', (2, 8))
        noise = self.config.get('noise', 0.8)
        invert = self.config.get('invert', 0.5)
        min_int = self.config.get('intensity', 0.2)

        # Using distractor generator logic if available, or standard
        # Using standard sample here for stability, or sample_with_distractors if preferred
        ...
        # --- KEY FIX: RANDOMIZED START TYPE ---
        # 50% chance of Cold Start (Simulate Inference)
        # 50% chance of Running Start (Maintain Momentum Training)
        if np.random.rand() < 0.5:
            # OPTION A: RUNNING START (Standard)
            # Start a bit into the line so we have history
            ...
            # OPTION B: COLD START (Inference Simulation)
            # Start at the very first pixel.
            # History is just the current point repeated (Zero Velocity).
            start_idx = 0
            curr = gt_poly[0]
    ...
    def step(self, a_idx: int):
        ...
        # Reward Logic (Gaussian Precision)
        if L_t < self.L_prev:
        ...
        # Slightly stricter off-track for cold start to prevent immediate spinning
        off_track = L_t > 10.0 
...
# ---------- TRAINING LOOP ----------
def train_phase6(args):
    print("--- STARTING PHASE 6: COLD START FINE-TUNING ---")
    print("Adapting agent to 'standstill' initialization vs 'running' initialization.")
    
    # Setup Env with moderate difficulty
    env = CurveEnvColdStart(h=128, w=128)
    env.set_config({'width': (2, 8), 'noise': 0.8, 'invert': 0.5, 'intensity': 0.2})
    
    K = 8
    nA = len(ACTIONS_8)
    model = AsymmetricActorCritic(n_actions=nA, K=K).to(DEVICE)
    
    # Load Previous Best (Phase 4 or Phase 5)
    try:
        print(f"Loading weights from: {args.load_path}")
        model.load_state_dict(torch.load(args.load_path, map_location=DEVICE))
        print("Weights loaded successfully.")
    except FileNotFoundError:
        print(f"ERROR: Model {args.load_path} not found. Please provide a valid Phase 4 or 5 checkpoint.")
        return

    # Use a VERY Low LR. We want to adapt the LSTM to the new start state,
    # NOT destroy the feature extractors we learned in previous phases.
    ...
    # We run fewer episodes because this is just a final polish
    ...
    for ep in range(1, TOTAL_EPISODES + 1):
        ...
        if ep % 100 == 0:
            ...
            # Success is defined as reaching the end (high positive reward)
            # Threshold varies based on path length, but >30 usually means success
            success_cnt = sum(1 for r in ep_returns[-100:] if r > 30)
            print(f"[Phase 6] Ep {ep} | Avg Rew: {avg_r:.2f} | Success: {success_cnt/100:.2f}")

        if ep % 500 == 0:
            save_name = f"ckpt_Phase6_ColdStart_ep{ep}.pth"
            torch.save(model.state_dict(), save_name)
            print(f"Saved checkpoint: {save_name}")

    final_name = "ppo_model_Phase6_Final.pth"
    torch.save(model.state_dict(), final_name)
    print(f"--- Phase 6 Complete. Model saved to {final_name} ---")

if __name__ == "__main__":
    p = argparse.ArgumentParser()
    p.add_argument("--episodes", type=int, default=10000, help="Episodes for fine-tuning")
    # Point this to your best previous model (Phase 5 preferred, or Phase 4)
    p.add_argument("--load_path", type=str, default="ppo_model_Phase5_final.pth")
    args = p.parse_args()
    train_phase6(args)
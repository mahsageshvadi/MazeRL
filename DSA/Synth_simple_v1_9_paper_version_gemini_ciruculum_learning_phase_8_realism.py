...
# ---------- PHASE 8 ENV: REALISM ADAPTATION ----------
class CurveEnvRealism:
    def __init__(self, h=128, w=128, max_steps=200):
        self.h, self.w = h, w
        self.max_steps = max_steps
        self.cm = CurveMakerFlexible(h=h, w=w, seed=None)
        # Wider range for width to match real vessels
        self.config = {'width': (1, 8), 'noise': 0.9, 'invert': 0.5} 
        self.reset()

    def generate_tissue_noise(self):
        """Generates 'cloudy' noise to mimic X-ray soft tissue."""
        noise = np.random.randn(self.h, self.w)
        # Gaussian blur creates "blobs" instead of static
        tissue = gaussian_filter(noise, sigma=np.random.uniform(2.0, 5.0))
        # Normalize to 0-1
        tissue = (tissue - tissue.min()) / (tissue.max() - tissue.min())
        return tissue * np.random.uniform(0.2, 0.5) # Scale intensity
    ...
    def reset(self):
        ...
        # 2. Add Tissue Noise (The "Realism" Upgrade)
        tissue = self.generate_tissue_noise()
        
        # Blend tissue into image based on inversion state
        # Check corner to guess if background is white or black
        is_white_bg = np.mean([img[0,0], img[0,-1]]) > 0.5
        
        if is_white_bg:
            # Dark tissues on white bg
            img = np.clip(img - tissue, 0.0, 1.0)
        else:
            # Light tissues on black bg
            img = np.clip(img + tissue, 0.0, 1.0)
        ...
        # 3. Smart Start (Looking ahead)
        p0 = gt_poly[0]
        p1 = gt_poly[min(4, len(gt_poly)-1)]
        vec_y, vec_x = p0[0] - p1[0], p0[1] - p1[1]
        p_minus_1 = (p0[0] + vec_y, p0[1] + vec_x)
    ...
    def obs(self):
        ...
        gt_obs = np.zeros((1, 33, 33), dtype=np.float32) # Dummy
        return {"actor": actor_obs, "critic_gt": gt_obs}

    def step(self, a_idx: int):
        self.steps += 1
        
        # Perturbation (Keep from Phase 7)
        ...
        if progress_delta > 0: r += precision_score * 2.0
        ...
        norm_gt = np.linalg.norm(gt_vec); norm_act = np.linalg.norm(act_vec)
        ...
        off_track = L_t > 8.0 
        done = reached_end or off_track or (self.steps >= self.max_steps)
...
def update_ppo(ppo_opt, model, buf_list, clip=0.2, epochs=4, minibatch=32):
    ...
    N = obs_a.shape[0]; idxs = np.arange(N)
...
def train_phase8(args):
    print("--- STARTING PHASE 8: TISSUE NOISE & REALISM ---")
    print("Adapting agent to 'cloudy' noise and varying structure.")
    
    env = CurveEnvRealism(h=128, w=128)
    ...
    batch_buffer = []; ep_returns = []; ep_successes = []
    ...
    for ep in range(1, args.episodes + 1):
        ...
        if len(ep_traj["rew"]) > 2:
            rews = np.array(ep_traj["rew"]); vals = np.array(ep_traj["val"] + [0.0])
            delta = rews + 0.9 * vals[1:] - vals[:-1]; adv = np.zeros_like(rews); acc = 0
            for t in reversed(range(len(rews))): acc = delta[t] + 0.9 * 0.95 * acc; adv[t] = acc
            ret = adv + vals[:-1]
            final_ep_data = {
                ...
                "logp": np.array(ep_traj["logp"]), "adv": adv, "ret": ret
            }
        ...
        if len(batch_buffer) >= 32:
            ...
            batch_buffer = []
        if ep % 100 == 0:
            print(f"[Phase 8] Ep {ep} | Avg Rew: {np.mean(ep_returns[-100:]):.2f} | Success: {np.mean(ep_successes[-100:]):.2f}")
        if ep % 1000 == 0: torch.save(model.state_dict(), f"ckpt_Phase8_ep{ep}.pth")

    torch.save(model.state_dict(), "ppo_model_Phase8_Realism.pth")

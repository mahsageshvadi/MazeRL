...
# --- FIX 1: SMART PADDING CROP ---
# This prevents "Black Borders" when the agent looks off the edge of a White image
...
def crop32(img: np.ndarray, cy: int, cx: int, size=CROP):
    ...
    # SMART PADDING LOGIC:
    # Sample the center pixel to guess the background intensity.
    # If we are on a white background (1.0), pad with 1.0.
    # If we are on a black background (0.0), pad with 0.0.
    center_val = img[min(max(0, cy), h-1), min(max(0, cx), w-1)]
    pad_val = 1.0 if center_val > 0.5 else 0.0
    
    # Initialize output with the background color (not just zeros!)
...
# ---------- ENVIRONMENT WITH INVERSION ----------
class CurveEnv:
    def __init__(self, h=128, w=128, max_steps=200):
        self.h, self.w = h, w
        self.max_steps = max_steps
        self.cm = CurveMaker(h=h, w=w, seed=None)
        self.reset()

    def reset(self):
        # --- CONFIGURATION FOR FINE-TUNING ---
        # 1. Width: 1 to 4 pixels (Standard vessels)
        # 2. Noise: 1.0 (Keep it robust against noise)
        # 3. Invert: 0.5 (CRITICAL: 50% chance of White Background)
        ...
        # Create GT Map (Centerline is always "Signal", regardless of bg color)
...
def resume_training(args):
    ...
    # 1. Initialize
    # We set seeds to something NEW so we get new random curves
    np.random.seed(999) 
    ...
    for ep in range(1, args.episodes + 1):
        ...
        if ep % 100 == 0:
            ...
            print(f"Fine-Tune Ep {ep} | Avg Rew: {avg_r:.2f} | Approx Success: {success_cnt/100:.2f}")

        if ep % 1000 == 0:
            save_name = f"ppo_inversion_fix_ep{ep}.pth"
...
if __name__ == "__main__":
    ...
    p.add_argument("--load_path", type=str, default="ppo_curve_agent.pth")
    args = p.parse_args()

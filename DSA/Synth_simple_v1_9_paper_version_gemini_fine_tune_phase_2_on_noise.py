...
class CurveEnv:
    ...
    def reset(self):
        ...
        # Width: 1.0 to 6.0 pixels
        # Noise: 1.0 (100% chance of noise)
        img, mask, pts_all = self.cm.sample_curve(width_range=(1.0, 6.0), noise_prob=1.0)
    ...
    def step(self, a_idx: int):
        ...
        # Relaxed threshold for noisy thick vessels
        on_curve = (L_t < 3.0) 
        ...
        off_track = L_t > 10.0 # Very relaxed for noise tolerance
...
def train_phase2(args):
    print("--- STARTING PHASE 2: FULL NOISE ADAPTATION ---")
    set_seeds(42)
    ...
    if args.load_path:
        try:
            ...
            print(f"Loaded Phase 1 Weights: {args.load_path}")
        except FileNotFoundError:
            print(f"ERROR: Could not find {args.load_path}")
            return
    else:
        print("ERROR: You must provide --load_path to the Phase 1 model!")
        return

    # Use 1e-4 because the images look VERY different now.
    # The CNN needs to learn new features (denoising).
    ...
    for ep in range(1, args.episodes + 1):
        ...
        if ep % 100 == 0:
            ...
            print(f"Phase 2 Ep {ep} | Avg Rew: {avg_r:.2f} | Success Rate: {success_rate:.2f}")

        if ep % 1000 == 0:
            save_name = f"ppo_phase2_final_ep{ep}.pth"
...
if __name__ == "__main__":
    ...
    p.add_argument("--load_path", default="ppo_phase1_width_ep5000.pth", type=str, help="Path to Phase 1 Model (e.g. ppo_phase1_width_ep5000.pth)")

...
parser.add_argument("--align_weight", type=float, default=2.0, help="Weight for Cosine Alignment Reward (Smoothness)")
parser.add_argument("--smooth_weight", type=float, default=0.2, help="Weight for Action Change Penalty (Jerk)")
parser.add_argument("--hidden_size", type=int, default=128, help="LSTM Hidden Size")
parser.add_argument("--seed", type=int, default=42, help="Random Seed")
parser.add_argument("--exp_name", type=str, default="default", help="Name for TensorBoard")
parser.add_argument("--gpu_id", type=int, default=0, help="GPU ID to use")
parser.add_argument("--scale", type=float, default=1.0, help="Multiplier for training duration (e.g. 10.0 for long run)")
...
def train():
    ...
    print(f"Training Scale Multiplier: {args.scale}x")

    env = CurveEnvRefined()
    model = ActorCriticSmooth(hidden_size=args.hidden_size).to(DEVICE)
    opt = torch.optim.Adam(model.parameters(), lr=args.lr)
    
    # Scale episodes by the user-defined multiplier
    mult = args.scale
    phases = [
        {'name': "Foundation", 'episodes': int(3000 * mult), 'cfg': {'width':(3,6), 'noise':0.0, 'stopping_training': False, 'distractors': False}},
        {'name': "Stopping_Clinic", 'episodes': int(3000 * mult), 'cfg': {'width':(3,6), 'noise':0.2, 'stopping_training': True, 'distractors': False}},
        {'name': "Robustness", 'episodes': int(5000 * mult), 'cfg': {'width':(2,6), 'noise':0.8, 'stopping_training': True, 'tissue': True, 'distractors': False}},
        {'name': "Distractors", 'episodes': int(5000 * mult), 'cfg': {'width':(2,5), 'noise':0.8, 'stopping_training': True, 'tissue': True, 'distractors': True}},
    ]

    total_ep = 0
    for phase in phases:
        print(f"=== PHASE: {phase['name']} ({phase['episodes']} eps) ===")
